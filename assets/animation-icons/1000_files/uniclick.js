...
var campaignIds = {
    'default': "63ca6afd4addf80001582972",
}
var cookieDomain = "litefinance.org"
...
var initialSrc = "https://stat.litefinance.org/" + campaignID + "?format=json" + "&referrer=" + referrer;
for (var i = 1; i <= 10; i++) {
...
if (!urlParams.get('rtkcid')) {
    ...
    rtkxhr.onreadystatechange = function () {
        if (rtkxhr.readyState == 4 && rtkxhr.status == 200) {
            ...
            if (sessionStorage.getItem('viewOnce') !== 1) {
                xhrr.open("GET", "https://stat.litefinance.org/view?clickid=" + rawData.clickid + "&referrer=" + referrer)
                xhrr.send();
            }
    ...
    if (sessionStorage.getItem('rtkclickid') === 'undefined' || !sessionStorage.getItem('rtkclickid')) {
        ...
        xhrr = new XMLHttpRequest;
        if (sessionStorage.getItem('viewOnce') !== 1) {
            xhrr.open("GET", "https://stat.litefinance.org/view?clickid=" + rtkClickID + "&referrer=" + referrer)
            xhrr.send();
        }
        if (registerViewOncePerSession) {
            sessionStorage.setItem('viewOnce', '1')
    ...
    if (sessionStorage.getItem('viewOnce') !== 1) {
        xhrTrack.open("GET", "https://stat.litefinance.org/view?clickid=" + rtkClickID + "&referrer=" + referrer)
        xhrTrack.send();
    }
    if (registerViewOncePerSession) {
        sessionStorage.setItem('viewOnce', '1')
